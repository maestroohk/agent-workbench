...
def _ensure_firstmate_shim(status: DependencyStatus) -> DependencyStatus:
    ...
    AGENTS.md operating manual, not a single binary. There is NO
    `bin/firstmate` entry script. The shim we write here is a
    dispatcher: it maps `firstmate <verb> [args]` to
    `fm-<verb>.sh [args]` in the harness's bin/. If no verb is given
    (i.e. `firstmate` with no args), it falls back to
    `fm-bootstrap.sh`, which is the harness's own entry point for a
    bare `firstmate` invocation. Unknown verbs (no matching
    `fm-<verb>.sh`) get a helpful error listing the available scripts,
    rather than a confusing "No such file" from bash.

    Note: the previous version of this shim exec'd a non-existent
    `bin/firstmate` file, which is why `firstmate --help` (or any
    bare `firstmate` call) would silently fail. See the `[Unreleased]`
    CHANGELOG entry for context.
    """
    from utils import AGENT_BIN_DIR
    ...
    # If a stale shim from a previous (buggy) install lives at
    # `firstmate` (no extension) on Windows, leave it alone — Windows
    # PATH lookup prefers the .cmd, and the bare shim is dead weight
    # that the user can remove themselves. We don't auto-delete files
    # outside the project root.
    legacy = AGENT_BIN_DIR / "firstmate"
    if legacy.is_file() and os.name == "nt":
        info(f"firstmate: legacy no-ext shim at {legacy} (harmless; .cmd takes precedence on Windows)")

    # Pick the right wrapper. The .cmd is needed on Windows because a
    # bare `firstmate` (no extension) cannot be exec'd there; the .cmd
    # invokes bash, which then runs the dispatcher below. On unix the
    # bash script is the dispatcher directly.
    if os.name == "nt":
        shim_path = AGENT_BIN_DIR / "firstmate.cmd"
        shim_path.write_text(
            "@echo off\r\n"
            f'bash "{harness_bin.as_posix()}/firstmate-shim.sh" %*\r\n',
            encoding="utf-8",
        )
        shim = shim_path
    else:
        shim_path = AGENT_BIN_DIR / "firstmate"
        shim_path.write_text(
            "#!/usr/bin/env bash\n"
            f'exec "{harness_bin.as_posix()}/firstmate-shim.sh" "$@"\n',
            encoding="utf-8",
        )
        shim_path.chmod(0o755)
        shim = shim_path

    # Drop the real dispatcher inside the harness. This is the script
    # that maps `firstmate <verb>` to `fm-<verb>.sh` and falls back to
    # `fm-bootstrap.sh` when no verb is given.
    dispatcher = harness_bin / "firstmate-shim.sh"
    dispatcher.write_text(
        "#!/usr/bin/env bash\n"
        "# firstmate shim: dispatches to fm-<verb>.sh in the harness's bin/.\n"
        "# Auto-generated by agent-workbench's _ensure_firstmate_shim hook.\n"
        "# Lives in the cloned repo so that reinstalls of the harness\n"
        "# overwrite it cleanly; the .cmd/.sh on PATH just execs this.\n"
        "set -e\n"
        f'here="{harness_bin.as_posix()}"\n'
        'if [ "$#" -eq 0 ]; then\n'
        '  verb="bootstrap"\n'
        'else\n'
        '  verb="$1"; shift\n'
        'fi\n'
        'target="${here}/fm-${verb}.sh"\n'
        'if [ ! -f "$target" ]; then\n'
        '  echo "firstmate: unknown verb: $verb" >&2\n'
        '  echo "firstmate: available fm-*.sh scripts:" >&2\n'
        '  ls "${here}"/fm-*.sh 2>/dev/null | xargs -n1 basename | sed "s/^fm-/  /;s/\\.sh$//" >&2\n'
        '  exit 64\n'
        'fi\n'
        'exec "$target" "$@"\n',
        encoding="utf-8",
    ...
    info(f"firstmate: shim installed at {shim} (dispatcher: {dispatcher})")
    return status
